...
class Runtime extends EventEmitter {
    ...
    _registerExtensionPrimitives (extensionInfo) {
        const existingCategory = this._blockInfo.find(info => info.id === extensionInfo.id);
        if (existingCategory) {
            // Extension scripts can register more than once. Keep one toolbox
            // category per extension ID and refresh it instead of appending a
            // duplicate each time.
            this._blockInfo = this._blockInfo.filter(info =>
                info.id !== extensionInfo.id || info === existingCategory);
            this._refreshExtensionPrimitives(extensionInfo);
            return;
        }
