...
class ExtensionManager {
    constructor (vm) {
        ...
        // Loading an extension is asynchronous. Track in-flight URLs so that
        // simultaneous project/UI requests share one load instead of starting
        // duplicate workers before workerURLs has been populated.
        this._loadingExtensionURLs = new Map();
    ...
    loadExtensionURL (extensionURL) {
        ...
        const pendingLoad = this._loadingExtensionURLs.get(extensionURL);
        if (pendingLoad) return pendingLoad;

        const load = this._loadExtensionURL(extensionURL);
        this._loadingExtensionURLs.set(extensionURL, load);
        const clearPendingLoad = () => this._loadingExtensionURLs.delete(extensionURL);
        load.then(clearPendingLoad, clearPendingLoad);
        return load;
    }

    async _loadExtensionURL (extensionURL) {
