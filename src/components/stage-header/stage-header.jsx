...
import addWindowIcon from './icon--add-window.svg';
import closeWindowIcon from './icon--close-window.svg';
...
const messages = defineMessages({
    ...
    addWindowMessage: {
        defaultMessage: 'Add game window',
        description: 'Button to add another independent game window beside the full screen stage',
        id: 'ninja.stageHeader.addWindow'
    },
    closeWindowMessage: {
        defaultMessage: 'Close game window',
        description: 'Button to close an extra game window',
        id: 'ninja.stageHeader.closeWindow'
    },
    openSettingsMessage: {
...
const isMultiplayerClient = new URLSearchParams(location.search).has('multiplayer');

const handleCloseWindow = () => {
    window.parent.postMessage({type: 'ninja-multiplayer-close'}, location.origin);
};
...
const StageHeaderComponent = function (props) {
    ...
    if (isFullScreen || isEmbedded) {
        ...
        const addWindowButton = isMultiplayerCell ? (
            <div className={classNames(styles.addWindowButton, styles.unselectWrapper)}>
                <Button
                    className={classNames(styles.stageButton, {[styles.disabled]: !canAddWindow})}
                    disabled={!canAddWindow}
                    onClick={onAddWindow}
                >
                    <img
                        alt={props.intl.formatMessage(messages.addWindowMessage)}
                        className={styles.stageButtonIcon}
                        draggable={false}
                        src={addWindowIcon}
                        title={props.intl.formatMessage(messages.addWindowMessage)}
                    />
                </Button>
            </div>
        ) : null;
        const fullscreenButton = isMultiplayerClient ? (
            <div className={styles.unselectWrapper}>
                ...
                >
                    <img
                        alt={props.intl.formatMessage(messages.closeWindowMessage)}
                        className={styles.stageButtonIcon}
                        draggable={false}
                        src={closeWindowIcon}
                        title={props.intl.formatMessage(messages.closeWindowMessage)}
                    />
                </Button>
            </div>
        ) : isFullScreen ? (
        ...
        header = (
            <Box
                className={classNames(styles.stageHeaderWrapperOverlay, {
                    ...
                    [styles.multiplayerCell]: isMultiplayerCell,
                    [styles.multiplayerClient]: isMultiplayerClient
                })}
            >
                <Box
...
StageHeaderComponent.propTypes = {
    ...
    isMultiplayerCell: PropTypes.bool,
