...
let pendingMultiplayerLoad = null;

const loadMultiplayerProject = async ({project, started, running}) => {
    if (!vm || !projectLoaded) {
        pendingMultiplayerLoad = {project, started, running};
        return;
    }
    await vm.loadProject(project);
    if (started || running) vm.start();
    if (running) vm.greenFlag();
};

window.addEventListener('message', event => {
    if (event.origin !== location.origin || !event.data || event.data.type !== 'ninja-multiplayer-load') return;
    loadMultiplayerProject(event.data);
});
...
const onProjectLoaded = () => {
    projectLoaded = true;
    if (pendingMultiplayerLoad) {
        const multiplayerLoad = pendingMultiplayerLoad;
        pendingMultiplayerLoad = null;
        loadMultiplayerProject(multiplayerLoad);
        return;
    }
