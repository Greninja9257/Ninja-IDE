...
import {CHANGED_EVENT, getHiddenExtensions, setExtensionHidden} from '../lib/ninja-hidden-extensions';
...
const messages = defineMessages({
    HIDE_EXTENSION: {
        defaultMessage: 'Hide',
        description: 'Context menu item that hides an extension category from the block palette',
        id: 'ninja.blocks.hideExtension'
    },
    PROCEDURES_RETURN: {
...
class Blocks extends React.Component {
    ...
    componentDidMount () {
        ...
        this.toolboxElement = this.workspace.getToolbox().HtmlDiv;
        this.toolboxElement.addEventListener('contextmenu', this.handleToolboxContextMenu);
    ...
    componentWillUnmount () {
        this.toolboxElement.removeEventListener('contextmenu', this.handleToolboxContextMenu);
        this.detachVM();
        this.unmounted = true;
    ...
    attachVM () {
        ...
        this.props.vm.addListener(CHANGED_EVENT, this.handleHiddenExtensionsChanged);
    ...
    detachVM () {
        ...
        this.props.vm.removeListener(CHANGED_EVENT, this.handleHiddenExtensionsChanged);
    ...
    getToolboxXML () {
        ...
        try {
            ...
            const hiddenExtensions = getHiddenExtensions(runtime);
            const dynamicBlocksXML = injectExtensionCategoryTheme(
                runtime.getBlocksXML(target).filter(category => !hiddenExtensions.includes(category.id)),
                this.props.theme
            );
    ...
    handleToolboxContextMenu (e) {
        ...
        if (!item) return;
        const idClass = Array.from(item.classList).find(name => name.startsWith('scratchCategoryId-'));
        const extensionId = idClass && idClass.slice('scratchCategoryId-'.length);
        if (!this.props.vm.runtime._blockInfo.some(category => category.id === extensionId)) return;
        e.preventDefault();
        e.stopPropagation();
        this.ScratchBlocks.ContextMenu.show(e, [{
            text: this.props.intl.formatMessage(messages.HIDE_EXTENSION),
            enabled: true,
            callback: () => setExtensionHidden(this.props.vm, extensionId, true)
        }], this.props.isRtl);
    }
