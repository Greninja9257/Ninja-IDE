...
import LocalCloudProvider from '../lib/local-cloud-provider';
...
const cloudManagerHOC = function (WrappedComponent) {
    class CloudManager extends React.Component {
        ...
        canUseCloud (props) {
            if (this.shouldUseLocalCloud(props)) return !props.cloudVariablesDisabledByUser;
            return !!(
        ...
        shouldUseLocalCloud (props) {
            return !props.isPlayerOnly || new URLSearchParams(location.search).has('multiplayer');
        }
        shouldConnect (props) {
        ...
        connectToCloud () {
            if (this.shouldUseLocalCloud(this.props)) {
                const params = new URLSearchParams(location.search);
                const sessionId = params.get('cloud_session') ||
                    (window.__ninjaLocalCloudSession ||
                        (window.__ninjaLocalCloudSession = Math.random().toString(36).slice(2)));
                this.cloudProvider = new LocalCloudProvider(this.props.vm, sessionId);
            } else {
        ...
        handleCloudDataUpdate (projectHasCloudData) {
            if (this.isConnected() && !projectHasCloudData) {
                ...
                // Local IDE simulation is expected and does not need a cloud
                // server notification.
                if (!this.shouldUseLocalCloud(this.props)) this.props.onShowCloudInfo();
                this.connectToCloud();
            }
        }
