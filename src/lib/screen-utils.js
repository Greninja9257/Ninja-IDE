...
const getStageDimensions = (stageSize, customStageSize, isFullScreen) => {
    ...
    if (isFullScreen) {
        const viewport = window.__ninjaStageViewport;
        const viewportHeight = viewport ? viewport.height : window.innerHeight;
        const viewportWidth = viewport ? viewport.width : window.innerWidth;
        stageDimensions.height = viewportHeight -
        ...
        const maxWidth = maxScaleParam ? (
            Math.min(viewportWidth, maxScaleParam * customStageSize.width)
        ) : viewportWidth - STAGE_DIMENSION_DEFAULTS.fullScreenSpacingBorderAdjustment;
